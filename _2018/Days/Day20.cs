...
namespace _2018.Days
{
    public class Day20 : Day
    {
        ...
        private void CalculateDoors(LinkedList<char> paths, Point pos)
        {
            while (paths.First.Value != '(')
            {
                Point? door = null;
                
                switch (paths.First.Value)
                {
                    case 'N':
                        door = new Point(pos.X, pos.Y - 1);
                        pos = new Point(pos.X, pos.Y - 2);
                        break;
                    case 'E':
                        door = new Point(pos.X + 1, pos.Y);
                        pos = new Point(pos.X + 2, pos.Y);
                        break;
                    case 'S':
                        door = new Point(pos.X, pos.Y + 1);
                        pos = new Point(pos.X, pos.Y + 2);
                    ...
                    case '$':
                        return;
                    default:
                        throw new ArgumentOutOfRangeException($"{paths.First.Value} is not one of NESW");
                }

                if (door.HasValue)
                {
                    this._doors.Add(door.Value);    
                }
                
                paths.RemoveFirst();
            }

            var nextPaths = GetPathOptions(paths);

            foreach (var nextPath in nextPaths)
            {
                this.CalculateDoors(nextPath, pos);
            }
        }

        private static IEnumerable<LinkedList<char>> GetPathOptions(LinkedList<char> paths)
        {
            var numBrackets = 1;
            var startNode = paths.First.Next;
            var pathOptions = new HashSet<LinkedList<char>>();
            var currentPath = new LinkedList<char>();

            while (numBrackets > 0 && startNode != null)
            {
                switch (startNode.Value)
                {
                    ...
                    case '(':
                        ...
                        break;
                    default:
                        currentPath.AddLast(startNode.Value);
                        break;
                }
        ...
        protected override void DoPart1()
        {
            ...
            this.CalculateDoors(paths, new Point(0, 0));
            
            ConsoleUtils.WriteColouredLine($"Found {this._doors.Count} doors", ConsoleColor.Blue);
        }
