...
public class Day7 : Day
{
    ...
    protected override void SolvePart1()
    {
        ...
        {
            if (dir.GetSize() <= 100000) totalSmallDirSize += dir.GetSize();

            var directoryChildren = dir.GetDirectoryChildren();

            foreach (var subDir in directoryChildren) queue.Enqueue(subDir);
        }
    ...
    protected override void SolvePart2()
    {
        ...
        while (queue.TryDequeue(out var dir))
        {
            ...
            if (dirSize < minSpaceToFree) continue;

            if (dirSize < candidateToDelete.GetSize()) candidateToDelete = dir;

            var childDirs = dir.GetDirectoryChildren();

            foreach (var subDir in childDirs) queue.Enqueue(subDir);
        }
